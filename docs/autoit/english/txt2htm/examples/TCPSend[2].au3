...
Func Example()
    ...
    If @error Then
        MsgBox(BitOR($MB_SYSTEMMODAL, $MB_ICONEXCLAMATION), "", "Server:" & @CRLF & "Invalid file chosen, Error code: " & @error)
        Return False
    EndIf
    ...
    If @error Then
        ...
        MsgBox(BitOR($MB_SYSTEMMODAL, $MB_ICONHAND), "", "Server:" & @CRLF & "Could not listen, Error code: " & @error)
        Return False
    EndIf
    ...
    Do ; Wait for someone to connect (Unlimited).
        ...
        ; If an error occurred display the error code and return False.
        If @error Then
            MsgBox(BitOR($MB_SYSTEMMODAL, $MB_ICONHAND), "", "Server:" & @CRLF & "Could not accept the incoming connection, Error code: " & @error)
            Return False
        EndIf
	...
	Do
		...
		; If an error occurred display the error code and return False.
		If @error Then
			MsgBox(BitOR($MB_SYSTEMMODAL, $MB_ICONHAND), "", "Server:" & @CRLF & "Could not send the data, Error code: " & @error)
