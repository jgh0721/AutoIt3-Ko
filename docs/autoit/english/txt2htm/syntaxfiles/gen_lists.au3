...
Func CreateLists()
	...
	If Not FileExists($KEYWORDLIST) Then ; In case it doesn't exist, but at the end must be removed.
		...
		FileWriteLine($hFileOpen, "Ternary")
		...
		FileWriteLine($hFileOpen, "#NoAutoIt3Execute")
...
Func DoFunctions($sFunctionPath, $sFunctionListOut, $sFunctionParamsOut, $sFunctionFullOut, $fUDFLibrary = False)
	...
	EndIf
	Local $sFunctionList = FileRead($hFileOpen), $sFunctionListParams = ""
	FileClose($hFileOpen)
	...
	Local $aSRE = StringRegExp($sFunctionList, $sSREPattern, 3), _ ; Parse the temp file for whole words at the start of a line i.e. functions.
			$sDescription = "", $sFunctionListParams = "", $sFunctionListFull = ""
	$sFunctionList = ""
...
Func DoMacros($sMacroListOut, $sMacroFullOut)
	...
	Local $hFileOpen = FileOpen("..\..\html\macros.htm", $FO_READ)
	If $hFileOpen = -1 Then
		Exit MsgBox($MB_SYSTEMMODAL, "AutoIt", "Error opening macros.html.")
	EndIf
	Local $sMacroHTML = FileRead($hFileOpen)
	FileClose($hFileOpen)
	Local $aSRE = StringRegExp($sMacroHTML, '<td>.*?>(@\w+)<.*?</td>', 3), _ ; Parse the html file. Macros appear in <a> tags.
			$aDescription = 0, _
			$sDescription = "Unknown", $sMacroList = "", $sMacroListFull = ""
	For $i = 0 To UBound($aSRE) - 1 ; Create a list of macros.
		$aDescription = StringRegExp($sMacroHTML, '\' & $aSRE[$i] & '</strong></td>\r\n\h*<td>(.*?)</td>', 3)
		$sDescription = "Unknown"
		If UBound($aDescription) Then $sDescription = $aDescription[0]
		$sMacroList &= $aSRE[$i] & @CRLF
		$sMacroListFull &= $aSRE[$i] & $ASCII_ACK & $sDescription & $ASCII_ACK & @CRLF
	Next
	...
	; Write the output
	$hFileOpen = FileOpen($sMacroListOut, $FO_OVERWRITE)
...
Func GetFunctionParams($sFilePath, ByRef $sDescription, $fUDFLibrary = False)
	...
	Local $aArray = StringSplit($sData, @CRLF, $STRINGSPLIT_ENTIRE), $iIndex = 0
	For $i = 1 To $aArray[0]
		Switch $aArray[$i]
			Case "###Description###"
